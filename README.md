...
#### Symbolic links for use with the Bash shell
...
#### Symbolic links for use with the zsh shell

    ln -s ~/dotfiles/zsh/zshenv ~/.zshenv
    ln -s ~/dotfiles/zsh/zprofile ~/.zprofile
    ln -s ~/dotfiles/zsh/zshrc ~/.zshrc
    ln -s ~/dotfiles/zsh/aliases.zsh ~/.oh-my-zsh/custom/aliases.zsh
    ln -s ~/dotfiles/git/git-completion.zsh ~/.git-completion.zsh
...
Nvim/Vim's backup, swap, and undo files are stored in `~/tmp/nvim` and
`~/tmp/vim`, respectively. Create the required directories with the following command:

    mkdir -p ~/tmp/{nvim,vim}/{.backup,.swp,.undo}

### Tmux note

On first run of tmux, remember to initialize your tmux plugins. Before starting
any sessions, enter `<prefix>+<shift>+<I>`.
